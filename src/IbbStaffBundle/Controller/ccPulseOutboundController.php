 ...
 */
class ccPulseOutboundController extends AbstractController
{
    /**
     * @Route("/ccPulse/queue-select", name="outbound_queue_select")
     ...
     * @Route("/ccPulse/queue-get-select-all", name="outbound_queue_get_select_all")
    ...
    public function queueGetSelectAll(Request $request, AgentStatusService $agentStatusService)
    {
        ...
        if ($queueName == "selectAll") {
            $users = $uspRepo->createQueryBuilder("up")
                ...
                ->setParameter("roles", "%ROLE_OUTBOUND%");
        } else {
...
//
            ...
            if (isset($userCallsData[$result['id']])) {
                ...
                $results[$key]["CallsTotalCount"]=$userCallsData[$result['id']]['Outbound']['cnt']??0;
            }
        ...
        $queueName = $request->get("tableId");
        if ($queueName == "selectAll") {
            $users = $em->getRepository(UserProfile::class)->createQueryBuilder("up")
                ->where("up.extension IS NOT NULL")
...
//                    $diffCallTime = $callTime->getDtHangUp()->getTimeStamp() - $callTime->getDtExten()->getTimeStamp();
    ...
    /**
     * @Route("/ccPulse/team-select", name="outbound_team_select_A")
     * @return \Symfony\Component\HttpFoundation\JsonResponse
     */
     ...
     * @Route("/ccPulse/team-get-select-all", name="outbound_team_get_select_all")
     * @param Request $request
     * @return JsonResponse
     * @throws \Exception
     */
...
//
            ...
            if (isset($userCallsData[$result['id']])) {
                $results[$key]["CallsTotalDuration"]= gmdate("H:i:s",$userCallsData[$result['id']]['Outbound']["total"]??0);
                $results[$key]["CallsAvarageDuration"]= gmdate("H:i:s",$userCallsData[$result['id']]['Outbound']['avarage']??0);
                $results[$key]["CallsTotalCount"]=$userCallsData[$result['id']]['Outbound']['cnt']??0;
...
//                ->select("COUNT(cl.idx)")
     ...
     * @Route("/ccPulse/agent-as-queue-get-select-all", name="outbound_agent_as_queue_get_select_all")
     * @param Request $request
     * @return JsonResponse
     * @throws \Exception
     */
...
//            $avail = $logedIn - $activeCalls - $inBreak - $inAcw;
     ...
     * @Route("/ccPulse/call-as-queue-get-select-all", name="outbound_call_as_queue_get_select_all")
     * @param Request $request
     * @return JsonResponse
     * @throws \Exception
     */
     ...
     * @Route("/ccPulse/summary",name="outbound_ccPulse_summary")
     * @return JsonResponse
...
///login sayısı total
        $lgnCount = $logins->createQueryBuilder("l");
        $lgnCount
            ->select("COUNT(l.id)")
            ->leftJoin("l.userId","u")
            ->where("u.state !=:state")
...
//            )
        ...
        $lgnCount = $lgnCount->getQuery()->getSingleScalarResult();
...
//            ->where("l.EndTime is not null")
        ...
        foreach ($publicTotalLoginTimes as $publicTotalLoginTime) {
            $diff = Date::diffDateTimeToSecond($publicTotalLoginTime->getLastOnline(), $publicTotalLoginTime->getStartTime());
