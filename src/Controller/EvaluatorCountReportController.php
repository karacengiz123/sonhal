...
class EvaluatorCountReportController extends AbstractController
{
    /**
     * @IsGranted("ROLE_CALL_DETAIL")
     * @Route("/evaluatorCountReportForm" ,name="evaluator-count-report-form")
...
//                    "1 Aylık Aralıklarla" => 30 * 24 * 60 * 60,
            ...
            ->getForm();
        return $this->render('evaluator_count_report/index.html.twig', [
            "form" => $form->createView(),]);
    }

    /**
     * @Route("/evaluatorCountReportTime")
     * @param Request $request
    ...
    public function igdas(Request $request)
    {
        ...
        if ($formValidate["form"]["firstDate"] == "" && $formValidate["form"]["lastDate"] == "") {
            ...
            if ($formValidate["form"]["firstDate"] == $formValidate["form"]["lastDate"] && $formValidate["form"]["time"] == 86400) {
                $rows = $this->evaluatorCountReport($firstTime, $lastTime);
                if (count($rows) > 0) {
                    foreach ($rows as $row) {
                ...
                foreach ($ranges as $range) {
                    ...
                    $rows = $this->evaluatorCountReport(date('Y-m-d H:i:s', $prevDate), date('Y-m-d H:i:s', $range - 1));
                    if (count($rows) > 0) {
                        foreach ($rows as $row) {
     ...
     * @return array
     */
    public function evaluatorCountReport($firsDate,$lastDate)
    {
