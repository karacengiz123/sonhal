...
function stateCheck() {
    ...
    if ([13,18].indexOf(parseInt(softVue.agentStatus)) > -1){
        return false;
    }

    $.ajax({
        type: 'GET',
        url: '/api/hardphone/state',
        headers: {
            'Authorization': "Bearer " + getTokenStorage(),
            'accept': "application/json",
            'Content-Type': "application/json",
...
function onRegisterSipEvent(extension) {
    if (tbxDebug == true) console.log("sip event girdi");
    $.ajax({
        type: 'GET',
        url: '/api/hardphone/on-register-sip-event/'+extension,
        headers: {
            'Authorization': "Bearer " + getTokenStorage(),
            'accept': "application/json",
            'Content-Type': "application/json",
        ...
        success: function (data) {
            if (data.hasOwnProperty('state')) {
                tokenVariableArray["extension"] = extension;
